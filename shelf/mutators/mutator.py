...
def mutate_linear_kaiming(module_linear, in_feature, out_feature):
    if isinstance(module_linear, torch.nn.Linear) is False:
        raise TypeError('The module must be a linear layer.')

    new_linear = torch.nn.Linear(in_feature, out_feature)

    torch.nn.init.kaiming_uniform_(new_linear.weight.data, a=math.sqrt(5))
    fan_in, _ = torch.nn.init._calculate_fan_in_and_fan_out(new_linear.weight.data)
    bound = 1 / math.sqrt(fan_in)
    torch.nn.init.uniform_(new_linear.bias.data, -bound, bound)
...
def mutate_conv2d_kaiming(module_conv2d, in_channel, out_channel):
    if isinstance(module_conv2d, torch.nn.Conv2d) is False:
        raise TypeError('The module must be a 2D convolutional layer.')

    new_conv2d = torch.nn.Conv2d(in_channel, out_channel, kernel_size=module_conv2d.kernel_size, stride=module_conv2d.stride, padding=module_conv2d.padding, dilation=module_conv2d.dilation, groups=module_conv2d.groups, bias=module_conv2d.bias is not None)

    torch.nn.init.kaiming_uniform_(new_conv2d.weight.data, a=math.sqrt(5))
    fan_in, _ = torch.nn.init._calculate_fan_in_and_fan_out(new_conv2d.weight.data)
    bound = 1 / math.sqrt(fan_in)
    torch.nn.init.uniform_(new_conv2d.bias.data, -bound, bound)
...
def mutate_batchnorm2d_identity(module_bn2d, num_channel):
    if isinstance(module_bn2d, torch.nn.BatchNorm2d) is False:
        raise TypeError('The module must be a batch normalization layer.')

    new_bn2d = torch.nn.BatchNorm2d(num_channel, eps=module_bn2d.eps, momentum=module_bn2d.momentum, affine=module_bn2d.affine, track_running_stats=module_bn2d.track_running_stats)
