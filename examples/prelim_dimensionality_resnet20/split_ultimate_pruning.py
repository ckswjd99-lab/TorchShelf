...
model_path_2 = './saves/set1_default/epoch_200.pth'
...
opt_dim_dict = {pname: diff / diff.norm() for pname, diff in weights_difference.items()}
...
optimizer = torch.optim.SGD(model.parameters(), lr=2e+0, momentum=0.9)
    ...
    for epoch in range(EPOCH):
        ...
        for input, label in pbar:
            ...
            for pname, param in model.named_parameters():
                param.data += opt_dim_dict[pname].data * SMOOTHING
...
plt.xlabel('Step')
plt.ylabel('Loss')
plt.title('Training Loss of Split-Ultimate Pruning')
plt.savefig(f'./logs/split_ultimate_pruning_tloss.png')
plt.close()
...
plt.xlabel('Step')
plt.ylabel('Accuracy')
plt.title('Training Accuracy of Split-Ultimate Pruning')
plt.savefig(f'./logs/split_ultimate_pruning_tacc.png')
plt.close()