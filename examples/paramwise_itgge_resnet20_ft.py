...
SAVE_PATH = './saves/paramwise_itgge_resnet/epoch_0900_0.6228.pt'
...
for epoch in range(START_EPOCH, START_EPOCH + NUM_EPOCH):
    ...
    for input, label in pbar:
        ...
        # estimated_gradient = gradient_fwd(input, label, model, functional_xent, query=int(num_params*0.01), type='rge')
        estimated_gradient = gradient_fwd(input, label, model, functional_xent, query=10, type='rge')
        ...
        pbar.set_description(f"Loss: {train_loss_avg:.4f}, Acc: {train_acc_avg*100:2.2f}")
    ...
    if epoch % 100 == 0:
        torch.save(model.state_dict(), f"./saves/paramwise_itgge_resnet/epoch_{epoch:04d}_ft.pt")
