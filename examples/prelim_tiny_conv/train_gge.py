...
for epoch in range(EPOCHS):
    ...
    for i, (input, label) in pbar:
        ...
        group_dict, group_sizes = group_by_gradient_exp(momentum_dict, num_groups)

        scaled_grads = {}
        estimated_gradient = gradient_fwd(
            input, label, model, functional_xent, 
            query=1, type='gge',
            group_dict=group_dict, group_sizes=group_sizes, num_groups=num_groups,
            scaled_grads=scaled_grads, cheat_fo=True
        )
        
        for pname, param in model.named_parameters():
            param.grad = scaled_grads[pname]
