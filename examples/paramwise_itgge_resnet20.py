    ...
    for pname, param in zip(param_names, param_list):
        ...
        if layerwise_pruning_ratio is not None:
            if 'weight' in pname:
                ...
                num_groups = param_size * (1 - PRUNING_RATE)
        else:
            num_groups = param_size * (1 - PRUNING_RATE)
...
def get_layerwise_pruning_ratio(model, dataloader, prune_ratio):
    ...
    fwd_grasp_score = get_zo_grasp_score(dataloader, model, query=TRIALS)

    # grasp_score_init = get_grasp_score(x, t, model)
    # importance_score_init_dict = { name: -score for name, score in zip(param_names, grasp_score_init) }
    importance_score_init_dict = { name: -score for name, score in zip(param_names, fwd_grasp_score.values()) }

    # prune model with grasp score
    global_unstructured_L1(model, importance_score_init_dict, prune_ratio)
...
base_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=NUM_EPOCH)
...
for epoch in range(NUM_EPOCH):
    ...
    layerwise_pruning_ratio = get_layerwise_pruning_ratio(model, train_loader, PRUNING_RATE)
    ...
    end_time = time.time()
    print(f"Epoch {epoch:4d}/{NUM_EPOCH:4d} : LR {learning_rate:.4e}, Query {config['total_query']} | CosSim: {cossim_avg:1.4f}, Training Loss: {train_loss_avg:.4f}, Training Acc: {train_acc_avg*100:2.2f}, Val Loss: {val_loss:.4f}, Val Acc: {val_acc*100:2.2f} | Time: {end_time - start_time:.2f}s")
