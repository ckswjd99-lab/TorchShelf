...
if os.path.exists('./saves/initial_model.pth'):
    model_resnet.load_state_dict(torch.load('./saves/initial_model.pth'))
else:
    torch.save(model_resnet.state_dict(), './saves/initial_model.pth')
...
for epoch in range(EPOCHS):
    ...
    if val_acc > best_val_acc:
        ...
        torch.save(model_resnet.state_dict(), './saves/best_model_running.pth')
...
for epoch in range(20):
    ...
    print(
        ...
        'Val Loss {val_loss:.3f}'
        .format(
            epoch + 1, 20, lr=epoch_lr, train_acc=train_acc, train_loss=train_loss, val_acc=val_acc, val_loss=val_loss
        )
    )
...
# load the best model
model_resnet.load_state_dict(torch.load('./saves/best_model_running.pth'))
val_acc, val_loss = validate(val_loader, model_resnet, criterion, epoch)
print(f'Best Validation Accuracy: {val_acc:.3f}')
torch.save(model_resnet.state_dict(), f'./saves/best_model_vacc{val_acc}.pth')
os.remove('./saves/best_model_running.pth')