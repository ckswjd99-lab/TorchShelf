...
scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=1, gamma=0.1 ** (1/100))
    ...
    for epoch in range(EPOCHS):
        ...
        for i, (input, label) in pbar:
            ...
            for q in range(QUERY):
                pnoises = {pname: torch.randn_like(param) for pname, param in model.named_parameters()}

                for pname, param in model.named_parameters():
                    if 'weight_orig' in pname:
                        pmask = dict(model.named_buffers())[pname.replace('_orig', '_mask')]
                        param.data += SMOOTHING * pnoises[pname] * pmask
                    else:
                        param.data += SMOOTHING * pnoises[pname]
                    ...
                    estimated_gradient[pname] += jvp_value * pnoises[pname]
            
            for pname, param in model.named_parameters():
                param.grad = estimated_gradient[pname] / QUERY
