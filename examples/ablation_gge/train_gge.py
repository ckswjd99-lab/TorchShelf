...
from shelf.trainers.zeroth_order import gradient_fwd, group_by_gradient_exp, gradient_estimate_randvec
...
def score_to_group_idx(score, r):
    score_sorted = torch.sort(score.view(-1), descending=False)[0]


def gradient_estimate_pwitgge(input, label, model, criterion, sparsity, smoothing=1e-3, real_gradient=None, init_momentum=None, layerwise_pruning_ratio=None, config=None):
    ...
    for pname, param in zip(param_names, param_list):
        ...
        num_groups = param_size * sparsity
        num_groups = int(np.ceil(num_groups) / 4)
...
# TEMP: test gradient estimate
input, label = next(iter(train_loader))
input, label = input.to(DEVICE), label.to(DEVICE)

config = {}
real_gradient = get_real_gradient(input, label, model, criterion)
estimated_gradient = gradient_estimate_pwitgge(input, label, model, criterion, 0.01, real_gradient=real_gradient, init_momentum=real_gradient, config=config)
rge_estimated = gradient_estimate_randvec(input, label, model, criterion, query=config['total_query'])

print(f"Total query: {config['total_query']}")
print(f"GGE gradient: {cossim_dict(real_gradient, estimated_gradient)}")
print(f"RGE gradient: {cossim_dict(real_gradient, rge_estimated)}")
...
for epoch in range(args.epoch):
    ...
    for i, (input, label) in pbar:
        ...
        config = {}
        real_gradient = get_real_gradient(input, label, model, criterion)
        estimated_gradient = gradient_estimate_pwitgge(input, label, model, criterion, 0.1, real_gradient=real_gradient, init_momentum=None, config=config)
        ...
        output = model(input)
        loss = criterion(output, label)
        train_acc += (output.argmax(1) == label).float().mean().item()
        train_loss += loss.item()

        total_query = config['total_query']

        pbar.set_description(f"Epoch {epoch+1:3d}/{args.epoch:3d} | Train Acc: {train_acc/(i+1)*100:.2f}, Train Loss: {train_loss/(i+1):.4f} | #Q {total_query}")
    ...
    print(f"Epoch {epoch+1:3d}/{args.epoch:3d} | Train Acc: {train_acc*100:.2f}, Train Loss: {train_loss:.4f}, Val Acc: {val_acc*100:.2f}, Val Loss: {val_loss:.4f} | #Q {total_query}, ETA: {epoch_end-epoch_start:5.2f}s")